...
function drawBlock(block) {
  ...
  // Bepaal kleur
    ...
    // Teken histogram als het een histogram blok is (BINNEN transformatie)
    if (block.isHistogram && block.histogram) {
        drawHistogram(block, block.isQuestion && block.hoverProgress > 0);
        pop();
        return; // Stop hier
    }
    
    // Teken tekst BINNEN de transformatie voor vraagblokken
    if (block.isQuestion) {
        fill(0);  // Zwarte tekst voor vraagblokken
        noStroke();
        textAlign(CENTER, CENTER);
        textStyle(BOLD);
        textSize(14);
        ...
        if (block.hoverProgress > 0) {
            // Bij hover: gebruik relatieve coördinaten (binnen transformatie)
    ...
    // Als het een vraagblok is, stoppen we hier (tekst is al getekend)
    if (block.isQuestion) {
        return;
    }
...
// Functie om histogram te tekenen - AANGEPAST om relatieve coördinaten te ondersteunen
function drawHistogram(block, useRelativeCoords = false) {
    let frequencies = block.histogram;
    let padding = 25;
    let chartWidth = CELL_SIZE - 2 * padding;
    
    let barWidth = chartWidth / 6;
    let chartHeight = barWidth * 6;
    let maxFreq = 6;
    
    // Bepaal basis coördinaten
    let baseX = useRelativeCoords ? 0 : block.x;
    let baseY = useRelativeCoords ? 0 : block.y;
    
    // Teken rasterlijnen (achtergrond)
    stroke(200, 200, 255, 100);
    strokeWeight(2);
    for (let i = 0; i <= maxFreq; i++) {
        let y = baseY + padding + chartHeight - (i / maxFreq) * chartHeight;
        line(baseX + padding, y, baseX + padding + chartWidth, y);
    }
    
    // Teken verticale rasterlijnen
    for (let i = 0; i <= 6; i++) {
        let x = baseX + padding + i * barWidth;
        line(x, baseY + padding, x, baseY + padding + chartHeight);
    }
    ...
    for (let i = 0; i < 6; i++) {
        ...
        let x = baseX + padding + i * barWidth;
        let y = baseY + padding + chartHeight - barHeight;
        rect(x, y, barWidth, barHeight);
    }
    
    // Omlijning om elke staaf
    stroke(128,128,128);
    strokeWeight(2);
    noFill();
    for (let i = 0; i < 6; i++) {
    ...
    stroke(105,105,105);
    strokeWeight(2);
    line(baseX + padding, baseY + padding, 
         baseX + padding, baseY + padding + chartHeight);
    line(baseX + padding, baseY + padding + chartHeight, 
         baseX + padding + chartWidth, baseY + padding + chartHeight);
    ...
    textSize(8);
    for (let i = 0; i < 6; i++) {
        let x = baseX + padding + i * barWidth + barWidth/2;
        let y = baseY + padding + chartHeight + 8;
        text(i + 1, x, y);
    }
    
    textSize(8);
    text("aantal", baseX + padding + chartWidth - 10, baseY + padding + chartHeight + 14);
    ...
    for (let i = 0; i <= maxFreq; i++) {
        let y = baseY + padding + chartHeight - (i / maxFreq) * chartHeight;
        text(i, baseX + padding - 4, y);
    }
    
    // "frequentie" label
    push();
    translate(baseX + padding - 13, baseY + padding + 2);
    rotate(-HALF_PI);
    textAlign(CENTER, CENTER);
    textSize(8);
    text("frequentie", 0, 0);
