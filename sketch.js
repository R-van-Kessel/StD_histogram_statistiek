...
function generateQuestions() {
    ...
    // Genereer 10 verschillende histogrammen
    for (let i = 0; i < 10; i++) {
        let frequencies;
        let modus;
        let maxFreq;
        
        // Blijf proberen tot we één unieke modus hebben
        do {
            frequencies = [];
            for (let j = 0; j < 6; j++) {
                frequencies.push(floor(random(0, 7))); // Frequentie 0-6
            }
            ...
            for (let score = 0; score < 6; score++) {
                if (frequencies[score] === maxFreq && maxFreq > 0) {
                    modus.push(score + 1);
                }
            }
        } while (modus.length !== 1); // Herhaal tot we precies 1 modus hebben
        ...
        for (let score = 0; score < 6; score++) {
            totaal += frequencies[score];
            som += (score + 1) * frequencies[score]; // Score is 1-6
            for (let k = 0; k < frequencies[score]; k++) {
                alleWaardes.push(score + 1);
            }
        }
        
        // Bereken gemiddelde (afgerond op 1 decimaal)
        let gemiddelde = totaal > 0 ? round(som / totaal * 10) / 10 : 0;
        gemiddelde = gemiddelde.toString().replace('.', ',');
        
        // Bereken spreidingsbreedte
        let min = -1, max = -1;
        for (let score = 0; score < 6; score++) {
            if (frequencies[score] > 0) {
                if (min === -1) min = score + 1;
                max = score + 1;
            }
        }
        let spreidingsbreedte = max - min;
        
        // Bereken mediaan
        alleWaardes.sort((a, b) => a - b);
        let mediaan;
        if (alleWaardes.length === 0) {
            mediaan = 0;
        } else if (alleWaardes.length % 2 === 0) {
            mediaan = (alleWaardes[alleWaardes.length/2 - 1] + alleWaardes[alleWaardes.length/2]) / 2;
        } else {
            mediaan = alleWaardes[floor(alleWaardes.length/2)];
        }
        mediaan = round(mediaan * 10) / 10;
        mediaan = mediaan.toString().replace('.', ',');
        
        let modusText = modus[0].toString();
        
        // Sla histogram data op
        questions.push({
            text: "HISTOGRAM",
            histogram: frequencies,
            answer: "gem:" + gemiddelde + "|spr:" + spreidingsbreedte + "|med:" + mediaan + "|mod:" + modusText,
            answerId: i
        });
        
        // Maak antwoord tabel
        let antwoordTekst = "Gemiddelde: " + gemiddelde + "\n" +
                           "Spreiding: " + spreidingsbreedte + "\n" +
                           "Mediaan: " + mediaan + "\n" +
                           "Modus: " + modusText;
    ...
    for (let row = 0; row < 2; row++) {
        for (let col = 0; col < COLS; col++) {
            blocks.push({
                ...
                histogram: questions[questionIndex].histogram,
...
function drawBlock(block) {
    ...
    // Teken histogram als het een histogram blok is
    if (block.isHistogram && block.histogram) {
        drawHistogram(block);
        return; // Stop hier, geen verdere tekst nodig
    }
    ...
    // Teken tekst - CHECK EERST OP BREUKEN
    if (block.text.includes('/')) {
        ...
        if (block.isRatioTable) {
            ...
            // Multi-line tekst (voor antwoordblokken met statistieken)
            textSize(11);
            textAlign(LEFT, CENTER);
            let lineHeight = 16;
            let startY = block.y + 25;
            for (let i = 0; i < lines.length; i++) {
                text(lines[i], block.x + 15, startY + i * lineHeight);
            }
        }
    } else if (block.isPercentage) {
...
// Functie om histogram te tekenen
function drawHistogram(block) {
    let frequencies = block.histogram;
    let padding = 25;
    let chartWidth = CELL_SIZE - 2 * padding;
    
    // AANPASSING 1: Maak vakjes vierkant - chartHeight gelijk aan barWidth × 6
    let barWidth = chartWidth / 6;
    let chartHeight = barWidth * 6; // Nu zijn de vakjes vierkant
    let maxFreq = 6;
    
    // Teken rasterlijnen (achtergrond)
    stroke(200, 200, 255, 100);
    strokeWeight(1);
    for (let i = 0; i <= maxFreq; i++) {
        let y = block.y + padding + chartHeight - (i / maxFreq) * chartHeight;
        line(block.x + padding, y, block.x + padding + chartWidth, y);
    }
    
    // Teken verticale rasterlijnen
    for (let i = 0; i <= 6; i++) {
        let x = block.x + padding + i * barWidth;
        line(x, block.y + padding, x, block.y + padding + chartHeight);
    }
    ...
    fill(255, 200, 100);
    for (let i = 0; i < 6; i++) {
        let barHeight = (frequencies[i] / maxFreq) * chartHeight;
        let x = block.x + padding + i * barWidth;
        let y = block.y + padding + chartHeight - barHeight;
        rect(x, y, barWidth, barHeight);
    }
    
    // AANPASSING 2: Omlijning om elke staaf
    stroke(128,128,128); // Licht grijze omlijning
    strokeWeight(1);
    noFill();
    for (let i = 0; i < 6; i++) {
        let barHeight = (frequencies[i] / maxFreq) * chartHeight;
    ...
    // Teken assen (over de staven heen)
    stroke(105,105,105); // donkergrijs
    strokeWeight(2);
    line(block.x + padding, block.y + padding, 
         block.x + padding, block.y + padding + chartHeight); // Y-as
    line(block.x + padding, block.y + padding + chartHeight, 
         block.x + padding + chartWidth, block.y + padding + chartHeight); // X-as
    
    // AANPASSING 3: Labels X-as (cijfers 1-6) - kleiner en niet bold
    fill(0);
    noStroke();
    textAlign(CENTER, CENTER);
    textStyle(NORMAL); // ← Niet vetgedrukt
    textSize(7); // ← Kleiner lettertype
    for (let i = 0; i < 6; i++) {
        let x = block.x + padding + i * barWidth + barWidth/2;
        let y = block.y + padding + chartHeight + 8;
        text(i + 1, x, y);
    }
    
    // Teken "aantal" onder X-as
    textSize(7);
    text("aantal", block.x + padding + chartWidth -10, block.y + padding + chartHeight + 14);
    
    // AANPASSING 4: Labels Y-as (cijfers 1-6) - kleiner en niet bold
    textAlign(RIGHT, CENTER);
    textStyle(NORMAL); // ← Niet vetgedrukt
    textSize(7); // ← Kleiner lettertype
    for (let i = 0; i <= maxFreq; i++) {
        let y = block.y + padding + chartHeight - (i / maxFreq) * chartHeight;
        text(i, block.x + padding - 4, y); // ← Cijfers 0-6
    }
    
    // Teken "frequentie" boven Y-as
    push();
    translate(block.x + padding - 13, block.y + padding + 2);
    rotate(-HALF_PI);
    textAlign(CENTER, CENTER);
    textSize(7);
    text("frequentie", 0, 0);
